/**
 * KundenstammDialog — pre-generated create/edit dialog for Kundenstamm.
 *
 ...
 * KundenstammDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Kundenstamm['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<KundenstammDialogDefaults | undefined>(undefined)
 */
...
/** Widened prefill type for KundenstammDialog.defaultValues — see file header. */
export type KundenstammDialogDefaults = Kundenstamm['fields'];
...
interface KundenstammDialogProps {
  ...
  defaultValues?: KundenstammDialogDefaults;
...
export function KundenstammDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KundenstammDialogProps) {
  ...
  const REQUIRED_FIELDS = ['firmenname', 'vorname', 'nachname', 'strasse', 'hausnummer', 'plz', 'ort'] as const;
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('kundenstamm') })
    : t('new_entity', { entity: appLabel('kundenstamm') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'firmenname': (
      <div key="firmenname" className="space-y-1.5">
        <Label htmlFor="firmenname">{fieldLabel('kundenstamm', 'firmenname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="firmenname"
        ...
        {showErrors && !fields.firmenname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'firmenname')}</p>
        )}
      </div>
    ),
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        <Label htmlFor="vorname">{fieldLabel('kundenstamm', 'vorname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="vorname"
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        <Label htmlFor="nachname">{fieldLabel('kundenstamm', 'nachname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="nachname"
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'nachname')}</p>
        )}
      </div>
    ),
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('kundenstamm', 'telefon')}</Label>
        <Input
          id="telefon"
          type="tel"
          inputMode="tel"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('kundenstamm', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
    ...
    'strasse': (
      <div key="strasse" className="space-y-1.5">
        <Label htmlFor="strasse">{fieldLabel('kundenstamm', 'strasse')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="strasse"
        ...
        {showErrors && !fields.strasse && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'strasse')}</p>
        )}
      </div>
    ),
    'hausnummer': (
      <div key="hausnummer" className="space-y-1.5">
        <Label htmlFor="hausnummer">{fieldLabel('kundenstamm', 'hausnummer')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="hausnummer"
          placeholder="z. B. 42a"
        ...
        {showErrors && !fields.hausnummer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'hausnummer')}</p>
        )}
      </div>
    ),
    'plz': (
      <div key="plz" className="space-y-1.5">
        <Label htmlFor="plz">{fieldLabel('kundenstamm', 'plz')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="plz"
        ...
        {showErrors && !fields.plz && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'plz')}</p>
        )}
      </div>
    ),
    'ort': (
      <div key="ort" className="space-y-1.5">
        <Label htmlFor="ort">{fieldLabel('kundenstamm', 'ort')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="ort"
        ...
        {showErrors && !fields.ort && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kundenstamm', 'ort')}</p>
        )}
      </div>
    ),
    'standort': (
      <div key="standort" className="space-y-1.5">
        <Label htmlFor="standort">{fieldLabel('kundenstamm', 'standort')}</Label>
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        <Label htmlFor="notizen">{fieldLabel('kundenstamm', 'notizen')}</Label>
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Zufahrt, Kontakt-Hinweise..."
  ...
  const FIELD_LABELS: Record<string, string> = {"firmenname": "Firmenname", "vorname": "Ansprechpartner Vorname", "nachname": "Ansprechpartner Nachname", "telefon": "Telefon", "email": "E-Mail", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "standort": "Standort (Kartenauswahl)", "notizen": "Notizen"};
